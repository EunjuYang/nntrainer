...
/**
 * @note Copy of `compile` from network_graph.cpp
 * @todo addLossLayer block should be updated.
 */
int SubGraphCpu::compile(const std::string &loss_type) {
  ...
  realizeInputOutputNode();

  /// @todo needs to be removed
  // only works when this subgraph is the end of the graph
  try {
...
/**
 * @note Copy of `initialize` from network_graph.cpp
 */
int SubGraphCpu::initialize(std::shared_ptr<Manager> tensor_manager_,
                            ExecutionMode mode,
                            const std::vector<Connection> &model_input_names,
                            const std::vector<Connection> &model_label_names) {

  tensor_manager = tensor_manager_;
  /**
  ...
  auto identify_as_model_input = [this](LayerNode *node) {
    ...
    input_name_list.push_back(node->getInput(0).getName());
  ...
  auto identify_as_model_label = [this](LayerNode *node) {
    ...
    output_name_list.push_back(node->getOutput(0).getName());
    label_name_list.push_back(node->getOutputGrad(0).getName());
  ...
  auto identify_external_tensors = [this](const std::vector<Connection> &conns,
    ...
    if (conns.empty()) {
      for (unsigned int i = 0; i < this->size(); ++i) {
        auto lnode = this->getSortedLayerNode(i).get();
        if (!pred(lnode)) {
          continue;
        }
...
int SubGraphCpu::reinitialize(
  ...
  auto identify_external_tensors = [this](const std::vector<Connection> &conns,
    ...
    if (conns.empty()) {
      ...
      for (auto &conn : conns) {
        auto lnode = this->getLayerNode(conn.getName()).get();
        NNTR_THROW_IF(!pred(lnode), std::invalid_argument)
          << "given node is not of that kind, name: " << conn.getName();
        identify(lnode);
      }
...
SubGraphCpu::forwarding(bool training,
                        ...
                        void *user_data, bool swap_mode) {

  for (auto iter = cbegin(); iter != cend() && !stop_cb(user_data); iter++) {
    auto &ln = *iter;
  ...
  for (unsigned int i = 0; i < getNumOutputNodes(); ++i) {
    auto const &output_layer_node = LNODE(getOutputNode(i));
...
bool SubGraphCpu::backwarding(
  ...
  for (iter_ = iter_begin; iter_ != iter_end && !stop_cb(user_data); iter_++) {
    ...
    PROFILE_TIME_START(profile_keys.at(ln->getType()));
    is_valid =
      backwarding_op(ln, iteration, stop_cb, user_data, is_grad_opt_mode);
    PROFILE_TIME_END(profile_keys.at(ln->getType()));
  ...
  if (!is_valid) {
    ...
    auto f_iter = cbegin() + getSortedNodeIdx((*iter_)->getName());
...
bool SubGraphCpu::backwarding_op(std::shared_ptr<LayerNode> node, int iteration,
  ...
  tensor_manager->flushCacheExcept(std::get<1>(node->getExecutionOrder()));
  ...
  tensor_manager->flushCacheExcept(std::get<2>(node->getExecutionOrder()));
...
void SubGraphCpu::applyGradients(LayerNode *node, int iteration) {
  ...
  for (unsigned i = 0; i < num_weight; ++i) {
    ...
    apply_grad_op(rc.getWeightObject(i), iteration);
  }
}

void SubGraphCpu::apply_grad_op(Weight &w, int iteration) {

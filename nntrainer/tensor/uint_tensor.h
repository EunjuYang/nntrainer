 ...
 * @file	uint_tensor.h
 * @date	02 April 2024
 * @brief	This is UIntTensor class for 16-bit unsigned integer calculation
...
/**
 * @class UIntTensor class
 * @brief UIntTensor class for T-bit unsigned integer calculation
 */
template <typename T>
class UIntTensor : public TensorBase {
public:

  /**
   * @brief Constructor for tdata_type
   */
  UIntTensor(){
    if(typeid(T) == typeid(uint8_t))
        tdata_type = Tdatatype::UINT8;
    else if(typeid(T) == typeid(uint16_t))
        tdata_type = Tdatatype::UINT16;
    else if(typeid(T) == typeid(uint32_t))
        tdata_type = Tdatatype::UINT32;
  }

...
void compute_fp16vcache_transposed(int row_num, const __fp16 *in,
  ...
  for (int n = head_start; n < actual_head_end; ++n) {
    ...
    std::vector<float16x8_t> sumVec(num_blocks * actual_gqa_count,
                                    vdupq_n_f16(0.0f));
    std::vector<__fp16> sumRem(actual_gqa_count * rem, 0.0f);
    ...
    for (int j = row_num < local_window_size ? 0
      ...
      for (int h = actual_gqa_start; h < actual_gqa_end; ++h) {
        ...
        for (int b = 0; b < num_blocks; ++b) {
          ...
          sumVec[h_local * num_blocks + b] =
            vfmaq_f16(sumVec[h_local * num_blocks + b], inVec, bVec);
        }

        __fp16 *remPtr = &sumRem.data()[h_local * rem];
    ...
    for (int h = actual_gqa_start; h < actual_gqa_end; ++h) {
      ...
      for (int b = 0; b < num_blocks; ++b) {
        ...
        vst1q_f16(&output[out_base], sumVec[h_local * num_blocks + b]);
      }

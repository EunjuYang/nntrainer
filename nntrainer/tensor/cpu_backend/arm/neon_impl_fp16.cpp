...
static void softmax_row_inplace_with_fp16_sink(__fp16 *qk_out, size_t start_row,
  ...
  for (size_t c = 0; c < full_blocks; c += 8) {
    ...
    for (size_t r = 0; r < row_range; ++r) {
      ...
      vst1q_f16(ptr, e); // In-place update: Replace score with exponentiated value
      sum = vaddq_f16(sum, e); // Accumulate sum
    }
    vst1q_f16(&sum_vals[c], sum);
  }

  // Handle remaining heads (scalar loop)
...
// Function to compute the context vector by multiplying Attention Probabilities (in) with Value Cache (vcache)
// Performs: Output = Attention_Probs * Value_Cache
// This version handles FP16 inputs/outputs and FP16 Value Cache.
// Optimized with NEON intrinsics for ARM architecture.
void compute_fp16vcache_transposed(int row_num, const __fp16 *in,
  ...
  // This loop structure was restored to handle all heads internally, reducing function call overhead.
  for (int n = 0; n < num_cache_head; ++n) {
    ...
    // Store the accumulated results to the output buffer
    for (int h = 0; h < gqa_size; ++h) {
      for (int b = 0; b < num_blocks; ++b) {
...
// Function to compute Attention Scores by multiplying Query (in) with Key Cache (kcache)
// Performs: Output = (Query * Key_Cache) / sqrt(Head_Dim)
// This version handles FP16 inputs/outputs.
// Optimized with NEON intrinsics for ARM architecture, including loop tiling.
...
// Implementation handles FP16 inputs/outputs.
void compute_rotary_emb_value(unsigned int width, unsigned int dim,
  ...
  for (unsigned int w = 0; w < width; w += dim) {
    ...
    // Check if we are doing out-of-place (output provided) or in-place operation
    if (output != nullptr) {
      ...
      for (; k + 7 < half_; k += 8) {
        ...
        // Load input pairs (x1, x2) separated by 'half_' (rotation usually happens between first and second half of head_dim)
        float16x8_t a = vld1q_f16(&inout[i0]);
        float16x8_t b = vld1q_f16(&inout[i1]);

        // Load precomputed Cosine and Sine values

...
inline static float16x8_t rcp_f16x8(float16x8_t x) {
  // apply reciprocal once
  float16x8_t rcp = vrecpeq_f16(x);
  // apply Newton-Rapson method several times
  // x_n+1 = x_n * (2 - x_0 * x_n)
  return vmulq_f16(rcp, vrecpsq_f16(x, rcp));
}
...
static void softmax_row_inplace_with_fp16_sink(__fp16 *qk_out, size_t start_row,
  ...
  for (size_t c = 0; c < vec_end; c += 8) {
    float16x8_t s = vld1q_f16(sum_vals + c);
    s = rcp_f16x8(s); // sum = 1/sum
    vst1q_f16(sum_vals + c, s);
  }
  for (size_t c = vec_end; c < num_heads; ++c) {
  ...
  for (size_t r = start_row; r < end_row; ++r) {
    __fp16 *row = qk_out + (num_heads * r);
    for (size_t c = 0; c < vec_end; c += 8) {
      float16x8_t s = vld1q_f16(sum_vals + c); // 1/sum
      float16x8_t v = vld1q_f16(row + c);      // exp(x - max)
      float16x8_t o = vmulq_f16(v, s);         // exp(x - max) * (1/sum)

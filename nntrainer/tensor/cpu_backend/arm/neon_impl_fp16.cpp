...
void softmax_row_inplace(__fp16 *qk_out, size_t start_row, size_t end_row,
  ...
  if (sink == nullptr) {
    ...
    return softmax_row_inplace_with_fp16_sink(qk_out, start_row, end_row, num_heads, sink);
  }
}
...
static void softmax_row_inplace_with_fp32_sink(__fp16 *qk_out, size_t start_row,
  ...
  for (size_t c = 0; c < full_blocks; c += 8) {
    ...
    for (size_t r = 0; r < row_range; ++r) {
      ...
      float16x8_t e_fp16 = vcombine_f16(vcvt_f16_f32(e_low), vcvt_f16_f32(e_high));
  ...
  for (size_t r = 0; r < row_range; ++r) {
    for (size_t c = 0; c < full_blocks; c += 8) {
      ...
      // Convert back to fp16 and store
      float16x8_t soft_fp16 = vcombine_f16(vcvt_f16_f32(soft_low), vcvt_f16_f32(soft_high));
...
void softmax_row(__fp16 *qk_out, size_t start_row, size_t end_row,
  ...
  if (sink == nullptr) {
    ...
    return softmax_row_with_fp16_sink(qk_out, start_row, end_row, num_heads, sink);
  }
}
...
static void softmax_row_with_fp32_sink(__fp16 *qk_out, size_t start_row,
  ...
  for (size_t r = start_row; r < end_row; ++r) {
    for (size_t c = 0; c < full_block; c += 8) {
      ...
      // Convert back to fp16 and store
      float16x8_t softmax_fp16 = vcombine_f16(vcvt_f16_f32(softmax_low), 
                                               vcvt_f16_f32(softmax_high));

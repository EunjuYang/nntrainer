 ...
 *            Default -1 means process all heads from head_start to
 *            num_cache_head. No other negative values are accepted.
 * @param[in] gqa_start start index of GQA group to process (default 0).
 *            Used for Q-head level parallelization during decoding.
 * @param[in] gqa_end end index (exclusive) of GQA group to process.
 *            Default -1 means process all groups [0, gqa_size).
 * @note Caller must ensure head_start < head_end when head_end != -1.
 */
 ...
 * @param[in] gqa_start start index of GQA group to process (default 0)
 * @param[in] gqa_end end index (exclusive) of GQA group (default -1 = all)
 * @note Caller must ensure head_start < head_end when head_end != -1.
 */

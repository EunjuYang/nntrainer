...
void compute_fp16vcache_fp32_transposed(int row_num, const float *in,
  ...
  __m256 *sumVec = new __m256[std::max(1, num_blocks * actual_gqa_count)];
  ...
  for (int n = head_start; n < actual_head_end; ++n) {
     ...
     */
    for (int i = 0; i < num_blocks * actual_gqa_count; i++) {
      sumVec[i] = _mm256_setzero_ps();
    }
    std::vector<float> sumRem((size_t)actual_gqa_count * rem, 0.0f);
      ...
      for (int h = actual_gqa_start; h < actual_gqa_end; ++h) {
        ...
        for (int b = 0; b < num_blocks; ++b) {
          ...
          sumVec[h_local * num_blocks + b] =
            _mm256_fmadd_ps(inVec, bVec, sumVec[h_local * num_blocks + b]);
        }
    ...
    for (int h = actual_gqa_start; h < actual_gqa_end; ++h) {
      ...
      for (int b = 0; b < num_blocks; ++b) {
        ...
        _mm256_storeu_ps(&output[out_base], sumVec[h_local * num_blocks + b]);
      }

      float *remPtr = &sumRem.data()[h_local * rem];
      // float *remPtr = &sumRem[h_local * rem];
      int base = num_blocks * 8;
      for (int r = 0; r < rem; ++r) {

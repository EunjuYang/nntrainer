...
UIntTensor<T>::UIntTensor(std::string name_, Tformat fm) {
    UIntTensor();
    TensorBase(name_, fm, tdata_type);
  }

template <typename T> UIntTensor<T>::UIntTensor(const TensorDim &d, bool alloc_now, Initializer init,
                         std::string name) :
...
template <typename T> UIntTensor<T>::UIntTensor(const TensorDim &d, const void *buf) :
  UIntTensor(d, true) {
  UIntTensor();
  if (d.getDataLen() != 0) {
    if (buf != nullptr)
...
UIntTensor<T>::UIntTensor(
  ...
  dim.setTensorType({fm, tdata_type});
...
template <typename T>
void UIntTensor<T>::deallocate() {
  data = nullptr;
  offset = 0;
}

template <typename T>
void *UIntTensor<T>::getData() const {
  if (!data)
    return nullptr;
...
template <typename T>
void *UIntTensor<T>::getData(size_t idx) const {
  if (!data)
    return nullptr;
...
template <typename T>
void *UIntTensor<T>::getAddress(unsigned int i) {
...
template <typename T>
const T &UIntTensor<T>::getValue(unsigned int i) const {
  return ((T *)getData())[i];
}

template <typename T>
T &UIntTensor<T>::getValue(unsigned int i) {
  return ((T *)getData())[i];
}

template <typename T>
const T &UIntTensor<T>::getValue(unsigned int b, unsigned int c,
...
template <typename T>
void UIntTensor<T>::setValue(float value) {
  T *data = (T *)getData();
...
template <typename T>
void UIntTensor<T>::setZero() {
  /// @todo replace with apply_i or scal
  setValue(0);
}

template <typename T>
void UIntTensor<T>::initialize() {
...
template <typename T>
void UIntTensor<T>::initialize(Initializer init) {
  initializer = init;
  initialize();
}

template <typename T>
void UIntTensor<T>::copy(const Tensor &from) {
  reshape(from.getDim());
  copy(from.getData());
}

template <typename T>
void UIntTensor<T>::copyData(const Tensor &from) {
...
template <typename T>
std::vector<unsigned int> UIntTensor<T>::argmax() const {
...
template <typename T>
float UIntTensor<T>::max_abs() const { return maxValue(); }

template <typename T>
float UIntTensor<T>::maxValue() const {
  const T *data = (T *)getData();
  return *std::max_element(data, data + size());
}

template <typename T>
float UIntTensor<T>::minValue() const {
  const T *data = (T *)getData();
  return *std::min_element(data, data + size());
}

template <typename T>
void UIntTensor<T>::print(std::ostream &out) const {
  const T *data = (T *)getData();
...
template <typename T>
void UIntTensor<T>::copy(const void *buf) {
  NNTR_THROW_IF(!contiguous, std::invalid_argument)
    << getName() << " is not contiguous, cannot copy.";

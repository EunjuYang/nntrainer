...
def save_qwen3_for_nntrainer(params, n_layers, dtype, file):
    ...
    # For tied embeddings, lm_head.weight may not exist in state_dict.
    # In that case, reuse embed_tokens.weight.
    if "lm_head.weight" in params:
        save_weight(params["lm_head.weight"].permute(1, 0))
    else:
        print("lm_head.weight not found (tie_word_embeddings=true), using embed_tokens.weight")
        save_weight(params["model.embed_tokens.weight"].permute(1, 0))

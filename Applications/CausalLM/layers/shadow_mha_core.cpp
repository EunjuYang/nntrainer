...
/**
 * @brief constructor of ShadowMHACoreLayer
 */
ShadowMHACoreLayer::ShadowMHACoreLayer() :
  shadow_mha_core_props(
    ...
    props::AttnLogitSoftcapping(), props::IsCausal(), props::SparseRatios()),
...
void ShadowMHACoreLayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  const unsigned int max_timestep =
    std::get<nntrainer::props::MaxTimestep>(shadow_mha_core_props).get();

  /** max position embeddings */
  unsigned int max_position_embeddings =
    std::get<props::MaxPositionEmbeddings>(shadow_mha_core_props).get();

  /** local window size */
  local_window_size =
    std::get<props::SlidingWindow>(shadow_mha_core_props).get();

  /** attention scaling computation */
  rope_scaling_type =
    std::get<props::RopeScalingType>(shadow_mha_core_props).get();
  scale = std::get<props::RopeScalingFactor>(shadow_mha_core_props).get();
  if (rope_scaling_type == "yarn")
    original_max_position_embeddings =
      std::get<props::RopeScalingMaxPositionEmbeddings>(shadow_mha_core_props)
        .get();
  ...
  num_heads_Q = static_cast<size_t>(
    std::get<nntrainer::props::NumHeads>(shadow_mha_core_props).get());
  num_heads_KV =
    std::get<props::NumHeads_KV>(shadow_mha_core_props).empty()
      ? num_heads_Q
      : static_cast<size_t>(
          std::get<props::NumHeads_KV>(shadow_mha_core_props).get());
  ...
  /** Weight for Sink */
  use_sink = std::get<props::UseSink>(shadow_mha_core_props).get();
  if (use_sink) {
  ...
  attn_logit_softcapping =
    std::get<props::AttnLogitSoftcapping>(shadow_mha_core_props).get();
  /** Is Causal */
  is_causal = std::get<props::IsCausal>(shadow_mha_core_props).get();

  /** Sparse Ratios */
  std::string ratios_str =
    std::get<props::SparseRatios>(shadow_mha_core_props).get();
  if (!ratios_str.empty()) {
    std::stringstream ss(ratios_str);
    std::string token;
    while (std::getline(ss, token, ',')) {
      parsed_sparse_ratios.push_back(std::stof(token));
    }
  } else {
    parsed_sparse_ratios =
      std::vector<float>(num_heads_Q, 1.0f); // Default to full attention
  }
  ...
  theta = (float)std::get<props::RopeTheta>(shadow_mha_core_props).get();
 ...
 * @date 2024-09-02
 */
void ShadowMHACoreLayer::forwarding(nntrainer::RunLayerContext &context,
                                    bool training) {}
 ...
 */
void ShadowMHACoreLayer::incremental_forwarding(
...
void ShadowMHACoreLayer::compute_kcaches(
...
void ShadowMHACoreLayer::one_batch_incremental_forwarding(
  ...
  if (to - from > 1) {
    ...
    for (int i = 0; i < seq_len; ++i) {
      futures.push_back(pool.submit_task([=, this, &out_]() {
        size_t start_row = 0;
        size_t end_row = 0;
        if (is_causal) {
        ...
        size_t row_len = end_row - start_row;

        if (out_.getDataType() == ml::train::TensorDim::DataType::FP32) {
          float *out_data = out_.getData<float>();
          for (size_t h = 0; h < num_heads_Q; ++h) {
            float ratio =
              h < parsed_sparse_ratios.size() ? parsed_sparse_ratios[h] : 1.0f;
            size_t k_keep = std::max((size_t)1, (size_t)(row_len * ratio));
            k_keep = std::min(row_len, k_keep);
            if (k_keep < row_len) {
              std::vector<float> scores(row_len);
              for (size_t r = 0; r < row_len; ++r) {
                scores[r] = out_data[(start_row + r) * num_heads_Q + h];
              }
              std::nth_element(scores.begin(), scores.end() - k_keep,
                               scores.end());
              float threshold = scores[row_len - k_keep];
              size_t kept = 0;
              for (size_t r = 0; r < row_len; ++r) {
                size_t idx = (start_row + r) * num_heads_Q + h;
                if (out_data[idx] >= threshold && kept < k_keep) {
                  kept++;
                } else {
                  out_data[idx] = -1e9f;
                }
          ...
          _FP16 *out_data = out_.getData<_FP16>();
          for (size_t h = 0; h < num_heads_Q; ++h) {
            float ratio =
              h < parsed_sparse_ratios.size() ? parsed_sparse_ratios[h] : 1.0f;
            size_t k_keep = std::max((size_t)1, (size_t)(row_len * ratio));
            ...
            if (k_keep < row_len) {
              std::vector<float> scores(row_len);
              for (size_t r = 0; r < row_len; ++r) {
                scores[r] = static_cast<float>(
                  out_data[(start_row + r) * num_heads_Q + h]);
              }
              std::nth_element(scores.begin(), scores.end() - k_keep,
                               scores.end());
              float threshold = scores[row_len - k_keep];
              ...
              for (size_t r = 0; r < row_len; ++r) {
                size_t idx = (start_row + r) * num_heads_Q + h;
                if (static_cast<float>(out_data[idx]) >= threshold &&
                    kept < k_keep) {
                  kept++;
                } else {
                  out_data[idx] = static_cast<_FP16>(-65504.0f);
                }
              }
            }
          }
 ...
 */
void ShadowMHACoreLayer::precompute_freqs(int head_dim, unsigned int seq_len,
                                          float theta) {
...
void ShadowMHACoreLayer::_compute_default_parameters(int head_dim,
                                                     float theta) {
...
void ShadowMHACoreLayer::_compute_yarn_parameters(int head_dim, float theta) {
...
void ShadowMHACoreLayer::apply_rotary_emb_tensor_v2(nntrainer::Tensor &in,
...
void ShadowMHACoreLayer::softmax_triangle(nntrainer::Tensor &qk_out, size_t row,
...
void ShadowMHACoreLayer::compute_fp16vcache_transposed(
...
void ShadowMHACoreLayer::setBatch(nntrainer::RunLayerContext &context,
                                  unsigned int batch) {

  const float dropout_rate =
    std::get<nntrainer::props::DropOutRate>(shadow_mha_core_props).get();
...
void ShadowMHACoreLayer::updateTensorsByInputDimensions(
  ...
  unsigned int &max_new_tokens =
    std::get<props::MaxNewTokens>(shadow_mha_core_props).get();
  unsigned int &max_position_embeddings =
    std::get<props::MaxPositionEmbeddings>(shadow_mha_core_props).get();
...
void ShadowMHACoreLayer::calcDerivative(nntrainer::RunLayerContext &context) {}

void ShadowMHACoreLayer::calcGradient(nntrainer::RunLayerContext &context) {}

void ShadowMHACoreLayer::exportTo(
  nntrainer::Exporter &exporter, const ml::train::ExportMethods &method) const {
  LayerImpl::exportTo(exporter, method);
  exporter.saveResult(shadow_mha_core_props, method, this);
}

void ShadowMHACoreLayer::setProperty(const std::vector<std::string> &values) {
  auto remain_props = loadProperties(values, shadow_mha_core_props);
  LayerImpl::setProperty(remain_props);
}

size_t ShadowMHACoreLayer::calc_attn_index(size_t i) {
  return (i * (i + 1)) / 2;
};

#ifdef PLUGGABLE

nntrainer::Layer *create_shadow_mha_core_layer() {
  auto layer = new ShadowMHACoreLayer();
  return layer;
}

void destroy_shadow_mha_core_layer(nntrainer::Layer *layer) { delete layer; }

extern "C" {
nntrainer::LayerPluggable ml_train_layer_pluggable{
  create_shadow_mha_core_layer, destroy_shadow_mha_core_layer};
}

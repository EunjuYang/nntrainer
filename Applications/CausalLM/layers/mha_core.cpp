  ...
  if (in.getDataType() == ml::train::TensorDim::DataType::FP32) {
    if (from) {
      ...
      for (unsigned int i = 0; i < sequence_len; ++i) {
        ...
        int row_to_compute = is_causal ? i + 1 : sequence_len;
        size_t out_start_row = 0;
        if (is_causal) {
          out_start_row = calc_attn_index(i);
        } else {
          out_start_row = i * sequence_len;
        }
    ...
    if (from) {
      ...
      nntrainer::compute_kcaches(in.getData<_FP16>(), cache.getData<_FP16>(),
                                 out.getData<_FP16>(), row_to_compute,
...
void MHACoreLayer::one_batch_incremental_forwarding(
  ...
  size_t width_ = 0;
  if (is_causal) {
    width_ = (from) ? to : calc_attn_index(to);
  } else {
    width_ = (to - from) * to;
  }
  nntrainer::Tensor out_(1, 1, width_, num_heads_Q, query_step.getTensorType());
  ...
  if (qk_out.getDataType() == ml::train::TensorDim::DataType::FP32) {
    ...
    if (from) {
      ...
      for (int i = seq - 1; i >= 0; --i) {
        ...
        if (is_causal) {
          start_row = calc_attn_index(i);
          end_row = calc_attn_index(i + 1);
        } else {
          start_row = i * row;
          end_row = (i + 1) * row;
        }

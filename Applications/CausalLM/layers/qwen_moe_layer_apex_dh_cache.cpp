...
void ApexMoeLayer::finalize(nntrainer::InitLayerContext &context) {
...
void ApexMoeLayer::incremental_forwarding(nntrainer::RunLayerContext &context,
  ...
  for (unsigned int b = 0; b < input_.batch(); ++b) {
    ...
    // Prefill-only
    auto total_results = router_logits.topK(num_experts);
...
void ApexMoeLayer::setProperty(const std::vector<std::string> &values) {
  auto remain_props = loadProperties(values, moe_props);
  nntrainer::LayerImpl::setProperty(remain_props);
}

void ApexMoeLayer::calcDerivative(nntrainer::RunLayerContext &context) {
...
void ApexMoeLayer::calcGradient(nntrainer::RunLayerContext &context) {
...
void ApexMoeLayer::updateTensorsByInputDimensions(

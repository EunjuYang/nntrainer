...
std::vector<LayerHandle> Qwen3ShadowTransformer::createAttention(
  ...
  std::vector<std::string> a_params = {
    ...
    withKey("sparse_ratios", sparse_ratios_str),
    withKey("input_layers", {Q_norm, K_norm, V})};
  layers.push_back(createLayer("shadow_mha_core", a_params));
...
void Qwen3ShadowTransformer::registerCustomLayers() {
  ///
  ...
  try {
    ...
    app_context->registerFactory(
      nntrainer::createLayer<causallm::ShadowMHACoreLayer>);
...
void Qwen3ShadowCausalLM::registerCustomLayers() {
  CausalLM::registerCustomLayers();
  Qwen3ShadowTransformer::registerCustomLayers();
}

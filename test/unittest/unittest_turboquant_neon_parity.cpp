 ...
 *
 * @file   unittest_turboquant_neon_parity.cpp
 * @date   22 April 2026
 * @brief  Parity tests between the scalar TurboQuant fallback and the NEON
 ...
 *
 *         Built only on ARM hosts; on non-ARM the NEON entry points do
 ...
 *        equality on the per-head L2 norm. The quantizer produces identical
 *        4-bit indices because (a) the FWHT butterfly order does not differ
 *        between scalar and NEON stages, and (b) the boundary comparison
 *        `val > b_i` is order-independent. The norm is computed via
 *        different reduction trees, so a small absolute tolerance is allowed.
 */
TEST(turboquant_neon_parity, quantize_kv_bitexact_nibbles) {
  for (int head_dim : {64, 128}) {
    for (int num_heads : {1, 4, 13}) {
      ...
      std::vector<uint8_t> p_fb(packed_bytes), p_nn(packed_bytes);
      std::vector<float> n_fb(num_heads), n_nn(num_heads);

      nntrainer::__fallback_quantize_kv_turboquant(
        input.data(), p_fb.data(), n_fb.data(), rot_signs.data(), head_dim,
        num_heads);
      nntrainer::neon::quantize_kv_turboquant(input.data(), p_nn.data(),
                                              n_nn.data(), rot_signs.data(),
                                              head_dim, num_heads);

      for (int b = 0; b < packed_bytes; ++b) {
        ASSERT_EQ(p_fb[b], p_nn[b])
          << "Nibble mismatch at byte " << b << " (head_dim=" << head_dim
          << ", num_heads=" << num_heads << ")";
      }
      for (int h = 0; h < num_heads; ++h) {
        EXPECT_NEAR(n_fb[h], n_nn[h], kNormAtol)
 ...
 *        and 4-way reduction-tree reordering but would still catch any
 *        single-bin nibble misalignment (which would change a score by
 *        ~ centroid_step * |rq[d]|, far above kFmaAtol).
 */
TEST(turboquant_neon_parity, compute_kcaches_matches_fallback) {
  ...
  for (const auto &f : cases) {
    ...
    std::vector<float> out_nn(out_n, 0.0f);
    ...
    nntrainer::neon::compute_kcaches_packed4(
      query.data(), packed.data(), norms.data(), out_nn.data(), f.num_rows,
      f.num_cache_head, f.head_dim, f.gqa_size, /*tile=*/1, rot_signs.data(),
      /*window=*/(size_t)f.num_rows, /*head_start=*/0, /*head_end=*/-1);
    ...
    for (size_t i = 0; i < out_n; ++i) {
      float d = std::fabs(out_fb[i] - out_nn[i]);
      if (d > max_abs_diff) {
        max_abs_diff = d;
        worst = i;
      }
...
/**
 * @brief Per-element parity for compute_vcache_packed4. Empirically
 *        bit-exact since the per-output-position accumulator keeps the
 *        same addition order as the scalar path; we still use kFmaAtol to
 *        remain robust against future micro-optimisations.
 */
TEST(turboquant_neon_parity, compute_vcache_matches_fallback) {
  const std::vector<KvFixture> cases = {
    {64, 4, 2, 12, 0x5555u},
    {64, 8, 4, 50, 0x6666u},
    {128, 2, 1, 7, 0x7777u},
  ...
  for (const auto &f : cases) {
    ...
    nntrainer::neon::compute_vcache_packed4(
      row_num, attn.data(), packed.data(), norms.data(), out_nn.data(),
      f.num_cache_head, f.gqa_size, f.head_dim, rot_signs.data(),
      /*window=*/(size_t)f.num_rows, /*head_start=*/0, /*head_end=*/-1);
 ...
 */
TEST(turboquant_neon_parity, kcaches_with_window_and_head_range) {
  ...
  nntrainer::__fallback_compute_kcaches_packed4(
    ...
    head_end);
  nntrainer::neon::compute_kcaches_packed4(
    query.data(), packed.data(), norms.data(), out_nn.data(), num_rows,
    num_cache_head, head_dim, gqa_size, 1, rot_signs.data(), window, head_start,
    head_end);
    ...
    for (int n = head_start; n < head_end; ++n) {
      for (int g = 0; g < gqa_size; ++g) {
        size_t idx = (size_t)t_row * num_cache_head * gqa_size +
                     (size_t)n * gqa_size + g;
        max_abs_diff =
          std::max(max_abs_diff, std::fabs(out_fb[idx] - out_nn[idx]));
      }
    }
  }
  EXPECT_LT(max_abs_diff, kFmaAtol);

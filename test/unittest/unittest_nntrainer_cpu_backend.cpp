...
TEST(nntrainer_cpu_backend_standalone, q4_0_repack_unpack_dequantize) {
  ...
  int64_t q4_0_block_size = QK4_0;
  int64_t q4_0_type_size = sizeof(block_q4_0_testonly);
  size_t data_size = (static_cast<size_t>(K) * N / q4_0_block_size) * q4_0_type_size;
  ...
  nntrainer::dequantize_row_q4_0(unpacked.data(),
                                 roundtrip_dequantized.data(),
                                 static_cast<int64_t>(K) * N);
...
/**
 * @brief Parameterised repack-unpack round-trip test over various dimensions.
 */
TEST(nntrainer_cpu_backend_standalone,
     q4_0_repack_unpack_dequantize_various_sizes) {
  nntrainer::init_backend();
  ...
  // N must be divisible by 8 (for q4_0x8 on x86) and K by 32 (QK4_0).
  TestCase cases[] = {
  ...
  for (const auto &tc : cases) {
    ...
    nntrainer::dequantize_row_q4_0(q4_weight.data(), ref.data(),
                                   static_cast<int64_t>(K) * N);

    // repack -> unpack -> dequantize
    std::vector<char> repacked(data_size);
    nntrainer::repack_q4_0(repacked.data(), q4_weight.data(), data_size, N, K);
    ...
    nntrainer::dequantize_row_q4_0(unpacked.data(), result.data(),
                                   static_cast<int64_t>(K) * N);

    for (unsigned int i = 0; i < N * K; ++i) {
      EXPECT_EQ(ref[i], result[i])
        << "Mismatch at index " << i << " for K=" << K << " N=" << N;
    }
  }
}
